...
build_wallet_classic() {
    ...
    if [ $BUILD_OPERATION_SYSTEM == "linux" ]; then
        tar -C $BUILD_DIR -zcf "${BUILD_DIR}/../../wallet_classic_${BUILD_NAME}.tar.gz" "${BUILD_DIR}/.."
    fi

    if [ $BUILD_OPERATION_SYSTEM == "win64" ] || [ $BUILD_OPERATION_SYSTEM == "win32" ]; then
        zip -rj "${BUILD_DIR}/../../wallet_classic_${BUILD_NAME}.zip" $BUILD_DIR
    fi
...
build_wallet() {
    ...
    if [ $BUILD_OPERATION_SYSTEM == "win64" ] || [ $BUILD_OPERATION_SYSTEM == "win32" ]; then
        zip -rj "${BUILD_DIR}/../../wallet_${BUILD_NAME}.zip" $BUILD_DIR
    fi
...
build_daemon() {
    ...
    if [ $BUILD_OPERATION_SYSTEM == "win64" ] || [ $BUILD_OPERATION_SYSTEM == "win32" ]; then
        zip -rj "${BUILD_DIR}/../../daemon_${BUILD_NAME}.zip" $BUILD_DIR
    fi
...
build_miner() {
    ...
    # cleanup
    BUILD_DIR="${BUILD_BASE_PATH}/miner"
    mkdir -p $BUILD_DIR
    lazbuild --os=$BUILD_OPERATION_SYSTEM --cpu=$BUILD_CPU --build-ide="-O$BUILD_OPTIMIZATION" --no-write-project pascalcoin_miner.lpi

    if [ $BUILD_OPERATION_SYSTEM == "win32" ] || [ $BUILD_OPERATION_SYSTEM == "win64" ]; then
        cp "PascalCoinMiner.exe" $BUILD_DIR
    else
        cp "PascalCoinMiner" $BUILD_DIR
    fi

    cp pascalsha.cl $BUILD_DIR
    cp ../README.md $BUILD_DIR
    cp ../LICENSE.txt $BUILD_DIR
    ...
    INFO_FILE="${BUILD_DIR}/../../miner_${BUILD_NAME}.txt"
    TIMESTAMP=$(date +%s)
    echo "TIME=${TIMESTAMP}" >> $INFO_FILE
    echo "BUILD_NAME=${BUILD_NAME}" >> $INFO_FILE
    echo "OS=${BUILD_OPERATION_SYSTEM}" >> $INFO_FILE
    ...
    if [ $BUILD_OPERATION_SYSTEM == "win64" ] || [ $BUILD_OPERATION_SYSTEM == "win32" ]; then
        zip -rj "${BUILD_DIR}/../../miner_${BUILD_NAME}.zip" $BUILD_DIR
    fi
